        ...
        def generate_xml
          super do |doc|
            ...
            @transactions.each do |transaction|
              ...
              ntry << Ox::Element.new("<AcctSvcrRef>#{              }</AcctSvcrRef>")
              ...
              bk_tx_cd << domn
              domn << Ox::Raw.new("<Cd>#{                      }</Cd>")

              fmly = Ox::Element.new(:Fmly)
              domn << fmly
              fmly << Ox::Raw.new("<Cd>#{                      }</Cd>")
              ...
              refs << Ox::Raw.new("<EndToEndId>#{                       }</EndToEndId>")
              ...
              id = Ox::Element.new(:Id)
              dbtr_acct << id
              id << Ox::Raw.new("<IBAN>#{                            }</IBAN>")

              rmt_inf = Ox::Element.new(:RmtInf)
              tx_dtls << rmt_inf
              rmt_inf << Ox::Raw.new("<Ustrd>#{                               }</Ustrd>")
