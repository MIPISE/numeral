...
describe "Numeral::V1::Reconciliations#create" do
  it "creates a new reconciliation" do
    unreconciled_payment_orders = Numeral::V1::PaymentOrders.get_list(uri_opt: {reconciliation_status: "unreconciled", connected_account_id: ENV["NUMERAL_SETTLEMENT_ACCOUNT_ID"], limit: 10}).dig("records")
    ...
    unreconciled_payment_orders.each do |payment_order|
      ...
      unreconciled_transaction = Numeral::V1::Transactions.get_list(uri_opt: {reconciliation_status: "unreconciled", amount_from: po_amount, amount_to: po_amount, connected_account_id: payment_order["connected_account_id"], limit: 1}).dig("records").first
