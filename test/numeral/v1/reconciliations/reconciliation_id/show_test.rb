...
describe "Numeral::V1::Reconciliations::ReconciliationId#get" do
  it "cancel a reconciliation" do
    reconciliations_list = Numeral::V1::Reconciliations.get_list(uri_opt: {limit: "1"})
    id = reconciliations_list["records"].first["id"]
    res = Numeral::V1::Reconciliations::ReconciliationId.get(id)
    assert res.is_a? Hash
    assert !res.dig("id").nil?
  end

  it "render error with invalid id" do
    res = Numeral::V1::Reconciliations::ReconciliationId.get("123456")
